...
saveOutput=function(obj,ofName=NA,saveWorkspace=F,oPath='.',caption=NA,rmdInd=NA, eval=T,objID=NA,header=NA,
    ...
    if(!is.null(obj)){
        ...
        if(!is.na(ofName)){
            ofName=paste0(counterStr,'.',ofName)
        }

        objPref=NULL
        if(is.data.frame(obj) || is.matrix(obj)){
            if(!is.na(ofName)){
                write.csv(obj, file=paste0(oPath,'/',ofName),quote=F,row.names=F,...)
            }

            objPref='tab'
        }else if(is.ggplot(obj)){
            if(!is.na(ofName)){
                ggsave(paste0(oPath,'/',ofName),obj,dpi=600)
            }
        ...
        if(saveWorkspace){
            ...
            save.image(paste0(oPath,'/',rImageFileName))
        }else{
            if(!is.na(ofName)){
                ofNameToken=unlist(strsplit(ofName,'.',T))
                ofNamePrefix=paste(ofNameToken[-length(ofNameToken)],collapse = '.')
                rImageFileName=paste0(ofNamePrefix,'.rds')
            }else{
                rImageFileName=paste0(counterStr,'.rds')
            }
            saveRDS(obj,paste0(oPath,'/',rImageFileName))
        }
        ...
        row1=list(oPath=oPath, rImageName=rImageFileName, ofName=ofName, caption=caption,
...
writeExcel=function(df1,fileName,...){
    #library(openxlsx)
    urls=file.path(df1[,"oPath"],df1[,"ofName"])
    names(df1[,"ofName"])=urls
    class(df1[,"ofName"])="hyperlink"
...
showObj=function(oDf,objID=NULL,isDocx=F,...){
    ...
    rImageFn = file.path(oDf[oInd,"oPath"],oDf[oInd,"rImageName"])
    if(grepl("\\.rds$",rImageFn)){
